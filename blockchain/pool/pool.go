...
func (tp *TxnPool) GetTxnByCount(num int, winningHash common.Uint256) (map[common.Uint256]*types.Transaction, error) {
	tp.RLock()
	defer tp.RUnlock()
	...
	exclusivedHashes, err := por.GetPorServer().GetTxnHashBySigChainHeight(blockchain.DefaultLedger.Store.GetHeight() +
