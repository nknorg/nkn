  ...
  export default {
    ...
    methods: {
      loadPage() {
        ...
        if (this.serviceStatus === ServiceStatusEnum.SERVICE_STATUS_NO_WALLET_FILE) {
          this.$router.push(this.localePath('wallet-create'))
        } else if (this.serviceStatus === ServiceStatusEnum.SERVICE_STATUS_NO_PASSWORD) {
          this.$router.push(this.localePath('wallet-open'))
        }else if (this.serviceStatus === ServiceStatusEnum.SERVICE_STATUS_RUNNING){
