...
type SigChain struct {
	rawdata     []byte         // traffic raw data
	rawDataSize uint32         // data size, used for verification
...
func New(rawdata []byte, srcPubkey *crypto.PubKey, destPubkey *crypto.PubKey, nextPubkey *crypto.PubKey) *SigChain {
	var chain SigChain
	chain.rawdata = rawdata
	chain.rawDataSize = uint32(len(rawdata))
...
func (p *SigChain) SerializationMetadata(w io.Writer) error {
	var err error
	err = serialization.WriteVarBytes(w, p.rawdata)
	if err != nil {
		return err
	}

	err = serialization.WriteUint32(w, p.rawDataSize)
	if err != nil {
		return err
	}
...
func (p *SigChain) DeserializationMetadata(r io.Reader) error {
	var err error
	rawdata, err := serialization.ReadVarBytes(r)
	if err != nil {
		return err
	}
	p.rawdata = rawdata

	rawDataSize, err := serialization.ReadUint32(r)
	if err != nil {
		return err
	}
