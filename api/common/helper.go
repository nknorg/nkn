...
func MakeDeleteNameTransaction(wallet vault.Wallet, name string, nonce uint64) (*types.Transaction, error) {
	account, err := wallet.GetDefaultAccount()
	if err != nil {
		return nil, err
	}
	...
	txn, err := types.NewDeleteNameTransaction(registrant, name, nonce)
	if err != nil {
		return nil, err
	}
...
func MakeSubscribeTransaction(wallet vault.Wallet, identifier string, topic string, bucket uint32, duration uint32, meta string, nonce uint64) (*types.Transaction, error) {
	account, err := wallet.GetDefaultAccount()
	if err != nil {
		return nil, err
	}
	...
	txn, err := types.NewSubscribeTransaction(subscriber, identifier, topic, bucket, duration, meta, nonce)
	if err != nil {
		return nil, err
	}
