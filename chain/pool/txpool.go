...
func (tp *TxnPool) GetTxnByCount(num int, winningHash common.Uint256) (map[common.Uint256]*Transaction, error) {
	...
	if winningHash.CompareTo(common.EmptyUint256) != 0 {
		if winningTxn, ok := tp.SigChainTxs[winningHash]; !ok {
...
func (tp *TxnPool) verifyTransactionWithLedger(txn *Transaction) error {
	...
	case pb.SubscribeType:
		...
		subscribed, err := chain.DefaultLedger.Store.IsSubscribed(pld.Subscriber, pld.Identifier, pld.Topic)
		if err != nil {
			return err
		}
		if subscribed {
		...
		subscriptionCount := chain.DefaultLedger.Store.GetSubscribersCount(pld.Topic)
		if subscriptionCount >= chain.SubscriptionsLimit {
