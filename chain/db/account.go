...
func (b *balance) Serialize(w io.Writer) error {
	...
	if err := serialization.WriteVarString(w, b.symbol); err != nil {
		return fmt.Errorf("balance symbol serialize error: %v", err)
	}
...
func (b *balance) Deserialize(r io.Reader) error {
	...
	b.symbol, err = serialization.ReadVarString(r)
	if err != nil {
		return fmt.Errorf("balance symbol deserialize error: %v", err)
	}
...
func (acc *account) SetBalance(assetID common.Uint256, symbol string, amount common.Fixed64) {
...
func (sdb *StateDB) SetBalance(addr common.Uint160, assetID common.Uint256, symbol string, value common.Fixed64) error {
	account, err := sdb.getAccount(addr)
	if err != nil {
		return err
	}

	account.SetBalance(assetID, symbol, value)
	return nil
}
...
func (sdb *StateDB) UpdateBalance(addr common.Uint160, assetID common.Uint256, symbol string, value common.Fixed64, op Operation) error {
	acc := sdb.GetOrNewAccount(addr)
	amount := acc.GetBalance(assetID)

	switch op {
	case Addition:
		acc.SetBalance(assetID, symbol, amount+value)
	case Subtraction:
		if amount < value {
